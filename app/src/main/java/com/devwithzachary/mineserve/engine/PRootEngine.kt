...
class PRootEngine(val context: Context) {
    ...
    val tmpDir: File get() = File(filesDir, "tmp").apply { 
        if (!exists()) mkdirs() 
        setWritable(true, false)
        setReadable(true, false)
        setExecutable(true, false)
    }
    val serversDir: File get() = File(filesDir, "servers").apply { if (!exists()) mkdirs() }
    ...
    fun isRootfsInstalled(): Boolean {
        ...
        val hasShell = binSh.exists() || usrBinSh.exists()
        return osRelease.exists() && hasShell && (dpkg.exists() || apt.exists() || libGlibc.exists())
    }
    ...
    fun buildPRootCommand(
        ...
        } else if (command == listOf("/bin/bash", "-l") && targetUser != "root") {
            listOf("/bin/su", "-", targetUser)
        } else {
            command
        }

        val targetWorkDir = if (config.workingDir == "/root" && targetUser != "root") {
            "/home/$targetUser"
        } else {
            config.workingDir
        }
        ...
        if (usePRoot) {
            ...
            // Mount servers directory into /servers and /home/mineserve/servers in rootfs
            if (serversDir.exists()) {
                try {
                    File(config.rootfsDir, "servers").mkdirs()
                    File(config.rootfsDir, "home/mineserve/servers").mkdirs()
                } catch (_: Exception) {}
                mounts.add("${serversDir.absolutePath}:/servers")
                mounts.add("${serversDir.absolutePath}:/home/mineserve/servers")
            }
            ...
            // Bind-mount synthetic /proc files to bypass Android 10+ SELinux restrictions
